...
def create_reservation(form_data):
    ...
    tables = Table.query.filter(Table.capacity <= form_data.num_guests.data).all()
    ...
    reservations = Reservation.query.join(Reservation.table).filter(Table.id.in_(t_ids), Reservation.reservation_time >= begin_range, Reservation.reservation_time <= end_range).all()
    if reservations:
        if len(t_ids) == len(reservations):
            ...
            # get available table
            table_id = (set(t_ids) - set([r.table for r in reservations])).pop()
            reservation = Reservation(guest=guest, table=Table.query.get(int(table_id)),
                                      num_guests=form_data.num_guests.data, reservation_time=form_data.reservation_datetime.data)
    else:
        # we are totally open
        reservation = Reservation(guest=guest, table=Table.query.get(int(t_ids[0])), num_guests = form_data.num_guests.data, reservation_time=form_data.reservation_datetime.data)
